...
def yoloDetect(uploaded_file,conf,iou,augment,device):
    ...
    with st.spinner('Wait for it...'):
        ...
        results = model(img_array,conf=conf,iou=iou,augment=augment,device=device)  # 不需要将 img_array 转换为 PIL Image
        result = results[0]       
        ...
        for i, box in enumerate(result.boxes):
            ...
            cv2.rectangle(img_array, (x1, y1), (x2, y2), BOX_COLOR, BOX_THICKNESS)  # 绘制边界框
            cv2.rectangle(img_array, (x1, label_y - label_height), (x1 + label_width, label_y), BOX_COLOR, cv2.FILLED) 
            cv2.putText(img_array, label, (x1, label_y), FONT_FACE, FONT_SCALE, FONT_COLOR, FONT_THICKNESS)
...
def palyground():
    ...
    user_check = st.selectbox("Please select the image or upload the picture", ['uploade img','img1', 'img2', 'img3'])
    if user_check == 'uploade img':
        uploaded_file = st.file_uploader("Choose a file", type=['bmp', 'png', 'jpg', 'jpeg', 'dng', 'tiff', 'webp', 'heic', 'tif', 'pfm'],key='upload_file')
   ...
   # 创建一个侧边栏，包含一些参数设置     
    with st.sidebar:
        st.header(':rainbow[Arguments setting]:+1:',anchor='hello',divider='blue',help='You can set some model arguments here')
        
        # 使用 st.session_state 中的值设置滑块初始值
        st.session_state.conf = st.slider('**confidence:**',min_value=0.0, max_value=1.0, step=0.01, value=st.session_state.conf,label_visibility='visible')
        st.session_state.iou = st.slider('**iou:**',min_value=0.0, max_value=1.0, step=0.01, value=st.session_state.iou,label_visibility='visible')
        
        st.markdown('**augment**')
        st.session_state.augment = st.toggle('**augment**',value=st.session_state.augment,label_visibility='hidden')
        
        st.button('check avliable devices',on_click=getDevice,use_container_width=True)
        st.session_state.device = st.selectbox('choose devices',st.session_state['devices'])
        ...
        if click_status:
            if st.session_state['original_img'] is None:
             st.warning('请上传图片', icon="⚠️")
                ...
                output_image = yoloDetect(st.session_state['original_img'],
